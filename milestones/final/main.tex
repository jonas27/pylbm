...
In this milestone I am interested in the effects of a certain $\omega$  on the shear wave decay and thus on the kinematic viscosity $\nu$ of a fluid. 
It is especially relevant in fluid dynamics and is defined as the ratio of the dynamic viscosity $\mu$ over the density $\rho$ of the fluid:
\begin{equation}
\nu = \frac{\mu}{\rho}
\end{equation}
The milestone is separated in three different parts. 
The general motion is to set a sinusoidal density or velocity profile and measure how fast it decays.
...
Plotting the normalized maximum velocity values along the y dimension and time on the x axis for $0.1 \leq \omega \leq 1.9 $ is shown in figure \ref{fig:m3-2-norm-vel}.
\begin{figure}[ht]
\centering
\resizebox{\columnwidth}{!}{\large\input{img/m3-2-shear-wave-decay.pgf}}
\vspace*{-10mm}
\caption[Normalized density decay]{The density decay for $0.1 \leq \omega \leq 1.9 $.  aster the amplitude decays. Left is the empirical decay and right}
\label{fig:m3-2-norm-vel}
\end{figure}
It illustrates that the higher the collision frequency the faster the system converges towards an equilibrium state. But contrary to the figure \ref{fig:m3-2-vel} it also shows that  increases in the collision frequency have a disproportional effect on the time it takes to reach the equilibrium state.


\begin{figure}[ht]
\centering
\resizebox{\columnwidth}{!}{\large\input{img/m3-3-kinematic-viscocity-scaling.pgf}}
\vspace*{-10mm}
\caption[Normalized amplitude decay]{The density decay with for $0.1 \leq \omega \geq 1.9 $. The less viscose the fluid the faster the amplitude decays.}
\label{fig:m3-1}
\end{figure}
