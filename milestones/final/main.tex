...
To emulate such a behaviour I use the following parameters:
\begin{enumerate}
...
To visualize the velocities over time I plot the single column over time, shown in figure \ref{fig:m4-1-velocities-over-time}.
\begin{figure}[ht]
\centering
\resizebox{0.7\columnwidth}{!}{\large\input{img/m4-1-velocities-over-time.pgf}}
\caption[Column velocities over time]{Column velocities over time}
\label{fig:m4-1-velocities-over-time}
\end{figure}
At first the velocities are high only at the moving lid, but after time, the velocities are 
