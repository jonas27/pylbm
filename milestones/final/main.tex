...
That the streaming works can visualized by the shifting on a 2D grid for each velocity.
\begin{figure}[ht]
\centering
\resizebox{\columnwidth}{!}{\large\input{img/m2-streaming.pgf}}
\vspace*{-1cm}
\caption[Overlapping protein-protein interactions]{Overlapping connections between  networks. By far the most connections are not shared between any network as depicted by the blue part. Only a total of $~13\%$ connections are shared and of those the majority are between two networks (orange part).}
\label{fig:ppi-comparison}
\end{figure}
